...
%diary('Diary_partial_17th_part.txt')
PLOT=0;
% starshot.IC.x=6500000;
% starshot.IC.y=0;
% starshot.IC.z=500000;
% starshot.IC.xdot=0;
% starshot.IC.ydot=7000;
% starshot.IC.zdot=0;
starshot.IC.x=7e6;
starshot.IC.y=0;
starshot.IC.z=0;
...
u=1;
tic
%discretization_c=linspace(0.1,0.5,50);
%discretization_Id=linspace(0.0001,0.1,50);
for k=1%:50
starshot.IC.eul=pi/18*[3 6 4.5];%-1+2*rand(1,3);%pi/6*[1 1 1];[-27.7875      39.0433     -28.1572]*pi/180;%
for N=1%1:30 
starshot.IC.w=[0.2 0.3 0.5];%-2+4*rand(1,3);%N/10*[1 1 1];[-2.1428     +2.5648      -3.2926];%
...
starshot.controller.T=60;%2*pi*sqrt((6371+starshot.controller.a)^3/398600);                % Orbital period
starshot.controller.tp=2*starshot.controller.T;                                     %   Peak Time
starshot.controller.ts=4*starshot.controller.T;                                     %   Settling Time
...
% Kane Damper
starshot.cmd.Id=0.0082551;%discretization_Id(N);%=eye(3)*0.001;
starshot.cmd.invId=inv(starshot.cmd.Id);
starshot.cmd.c=0.41837;%discretization_c(k);

starshot.cmd.w=[0 0 1]';
starshot.cmd.wdb0=0;%[0 1 2];%-starshot.IC.w;%norm(starshot.IC.massproperties.I*starshot.cmd.w)*starshot.cmd.invId(1,1)*[0 0 1]';
%% Drag
...
starshot.magnetorq.e=.04;%0.33;                            % Ampere
...
starshot.model='StarshotAC_DC_kd12';
load_system(starshot.model)
sim_time=7000;
starshot.output=sim(starshot.model,'SimulationMode','normal','StopTime','sim_time');
...
if PLOT==1
    figure (2)
    subplot(3,1,1)
    plot(starshot.output.w.time,starshot.output.w.data(:,1))
    grid on
    subplot(3,1,2)
    plot(starshot.output.w.time,starshot.output.w.data(:,2))
    grid on
    subplot(3,1,3)
    plot(starshot.output.w.time,starshot.output.w.data(:,3))
    grid on
    

    figure (3)
    plot(starshot.output.real_angle.time,starshot.output.real_angle.data)
    grid on
end
...
figure (4)
plot(starshot.postproc.W(:,1),starshot.postproc.W(:,2),'-')
grid on
grid minor
hold on
plot(starshot.IC.massproperties.PI(1,3),starshot.IC.massproperties.PI(2,3),'.','Markersize',20)
hold off
...
%dlmwrite('c_0p1to0p5__1to40__Id0p0001to0p1__11to30.csv',starshot.postproc.performance.results)
